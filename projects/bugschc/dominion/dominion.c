...
int playFeast(struct gameState *state, int handPos, int currentPlayer, int choice1) {
	int i;
	int x;
	int temphand[MAX_HAND];// moved above the if statement
...
int playTribute(struct gameState *state, int currentPlayer, int handPos) {
	int i;
...
int playSeaHag(struct gameState *state, int currentPlayer, int handPos) {
	int i;
	for (i = 0; i < state->numPlayers; i++) {
		if (i != currentPlayer) {
