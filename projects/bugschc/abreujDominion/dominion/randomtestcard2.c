...
int testCardEffect(struct gameState* post, int handpos, int results[5]) {
	...
	great_hallEffect(post, 0, handpos);
	...
	//test last card in discard deck is great hall card
	if (post->discard[0][post->discardCount[0] - 1] != great_hall) {
		results[3]++;
	}

	//test # of actions increased by 1
